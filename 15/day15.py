...
def main():
    ...
    with open( Path(__file__).with_name( "input.txt" ), 'r' ) as file:
        for line in file.readlines():
            ...
            rawData[ (int(sensorX), int(sensorY), int(beaconX), int(beaconY))] = manhattanDistance( int(sensorX), int(sensorY), int(beaconX), int(beaconY) )
    ...
    part1Result = (
        max( sensorBeacon[0] - abs(part1Target-sensorBeacon[1]) + distance for sensorBeacon, distance in rawData.items() )
        - min( sensorBeacon[0] + abs(part1Target-sensorBeacon[1]) - distance for sensorBeacon, distance in rawData.items() )
    )
    ...
    for sensorBeacon, distance in rawData.items():
        sensorX, sensorY, beaconX, beaconY = sensorBeacon
